(function (root) {
    ...
    //strategy API object
    var StrategyObjectAPI = root.StrategyObjectAPI = function (strategyName) {
        var instance = this;
        instance.strategyName = strategyName;
        instance.requester = angular.requests;
        angular.events.once('ApiRequestLoaded', function () {
            instance.requester = instance.requester || angular.requests;
        });
    }
    StrategyObjectAPI.default = new StrategyObjectAPI('default');
    StrategyObjectAPI.prototype = {
        setRequester: function (angularRequester) {
            if (!(angularRequester instanceof ApiRequester))
                throw 'Requester must be ApiRequester';
            this.requester = angularRequester;
        },
        /**
        ...
        getStrategyRow: function (rowsArray, includeData, includeFile, callback) {
            ...
            async.map(rowsArray, function (row, doneOne) {
                ...
                instance.requester.get('/api/kafka/GetRow', row, function (err, r) {
                    if (!err) {
        ...
        getStrategyRowList: function (callback) {
            var instance = this;
            instance.requester.get('/api/kafka/GetStrategy', {
                strategyName: instance.strategyName
            }, callback);
        },
        /**
        ...
        addNewRow: function (rowText, callback) {
            var instance = this;
            instance.requester.get('/api/kafka/AddRow', {
                row: rowText,
                strategyName: instance.strategyName
            }, function (err, data) {
                if (err) callback(err);
         ...
         * 
         * */
        deleteStrategyDialog: function (reloadPage, callback) {
            var instance = this;
            angular.showMessage({
                ...
                size: 'sm',
                ok: function (settings, close) {
                    instance.requester.post('/api/kafka/ClearStrategy', {
                        strategyName: instance.strategyName
                    }, function (err) {
                        if (err) return alert(err);
                        close();
        ...
        /**
         * Upload the series points to SW
         * 
         * @param {Array} series: The series points
         * */
        uploadSeriesToSW: function (seriesName, seriesPoints, overwrite, callback) {
            var instance = this;
            var x, y, data;
            data = seriesPoints.map(function (p) {
                x = moment.utc(p.x).format('YYYYMMDDTHHmm');
                y = Number.isFinite(p.y) ? p.y : 'NA';
                return [x, y]
            });

            instance.requester.post('/api/kafka/SetTimeSeries', {
                strategyName: instance.strategyName,
                timeSeriesName: seriesName,
                data:data,// LZString.compressToBase64(JSON.stringify(data)),
                //compressed: true,
                overwrite: overwrite
            }, callback);
        },
...
(function (root) {
    ...
    //Add functions as static
    angular.extend(StrategyRowInput, {
         ...
         * */
        of: function (rowInput, moreProperties) {
            var obj;
            if (typeof (rowInput) == 'object') {
                ...
                if (Number.isFinite(rowInput.panelDisplayIndex))
                    obj.panelDisplayIndex = rowInput.panelDisplayIndex;
                if (rowInput.panelTitle)
                    obj.panelTitle = rowInput.panelTitle;
         ...
         * */
        ofArray: function (rowInput, moreProperties) {
            if (Array.isArray(rowInput))
                return rowInput.map(function (row) {
                    return StrategyRowInput.of(row, moreProperties);
                });
            if (typeof (rowInput) == 'object') {
